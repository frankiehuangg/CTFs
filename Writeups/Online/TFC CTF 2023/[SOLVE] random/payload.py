...
# LOCAL
file = FILE = "./random"
libc = LIBC = CDLL(find_library('c'))
if (FILE != ""):
    elf = ELF = context.binary = ELF(FILE, checksec=False)

# REMOTE
SERVER = "challs.tfcctf.com"
PORT = 31786
...
numbers = []
for i in range(10):
    numbers.append(str(libc.rand()).encode())

for i in range(10):
    io.sendline(numbers[i])
